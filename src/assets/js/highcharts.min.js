$(function() {
	$('#container').highcharts({
		...
		tooltip: {
			shared: true,
			/*pointFormat: '温度:{point.y}_25E2_2584_2583',*/
			/*pointFormat: '公布值:{point.y}',*/
			formatter: function() {
				var content = '<span style="font-size: 10px;">' + '</span>';
				var date = new Date();
				content += '<span>' /*+ date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDay()*/ + ' ' + (date.getMinutes() + 1) + ':' + (date.getSeconds() + 1) + ':' + (date.getHours() + 1) + '</span>'+'<br>';
				for (var i = 0; i < this.points.length; i++) {
					content += '<span style="padding-top:15px;' + this.points[i].series.color + '">' + '</span>公布值:' + this.points[i].y;
				};
				return content;
